...
def test_load_sjr_csv_skips_truncated_rows(db_session):
    """Test that truncated CSV rows (with fewer fields than header) are skipped without crashing.

    When csv.DictReader encounters a row with fewer comma-separated values than the header,
    it fills missing trailing fields with None. This would cause TypeError (int(None)) or
    AttributeError (None.strip()) without proper exception handling.
    """
    count = load_sjr_csv(db_session, "tests/fixtures/sjr_sample_with_truncated_row.csv")
    db_session.flush()

    # Should load 2 valid rows and skip 1 truncated row
    assert count == 2
    assert db_session.query(JournalSJR).count() == 2
