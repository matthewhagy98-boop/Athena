...
| **RESOLVED 2026-08-03 (commit `a5a4117`).** `select_due_users` originally called `list_interests` and `get_delivery_preference` *outside* its `try` block; both end in `.scalar_one()`, so either raised `NoResultFound` for a user missing the corresponding row | [EXISTING] | `digest/runner.py:30,32` (pre-fix); `digest/profiles.py:26,69` | This was a **live** defect, not a latent one — see the corrected note below |
...
**Correction to the original draft of this entry.** It asserted that `select_due_users` already caught `NoResultFound` from `list_interests` and carried an explanatory comment. It did not — that was an error in the first draft of this document, and the line citations were off by six lines. The true pre-fix code called **both** `list_interests` (`digest/runner.py:30`) and `get_delivery_preference` (`digest/runner.py:32`) outside the `try` block, with no such guard or comment anywhere.

The defect was therefore **worse and already live**, not latent and not dependent on C. Sub-project A's `POST /users/anonymous` creates users with `status='active'` and no `InterestProfile`, so `list_interests` raised on the *first* of them. Eight such users are committed in the dev database, and six digest runner tests were failing on `main` as a direct result. `run_all_due_digests` calls `select_due_users` under `try`/`finally` with no `except` (`digest/runner.py:85-87`), so the exception aborted delivery for every user.
