...
| **RESOLVED 2026-08-03 (commit `a5a4117`).** On `codex/frontend-shell` only `list_interests` was guarded, leaving `get_delivery_preference` (`:38`) exposed; on `main` neither was guarded. Both end in `.scalar_one()`, so either raised `NoResultFound` for a user missing the corresponding row | [EXISTING] | `codex/frontend-shell` `digest/runner.py:31-38`; `main` `digest/runner.py:30,32`; `digest/profiles.py:26,69` | Live on `main`, still reachable on the reference branch — see the provenance note in §22 |
| `run_all_due_digests` calls `select_due_users` under `try`/`finally` with **no** `except` | [EXISTING] | `digest/runner.py:85-87` | An exception escaping `select_due_users` aborted the digest run for every user |
| Confirmed empirically: 8 committed `anon-*@no-reply.local` users with `status='active'` and no `InterestProfile` exist in the dev database, and 6 digest runner tests failed on `main` because of them | [EXISTING] | `tests/digest/test_runner.py`; verified 2026-08-03 against the dev database | Sub-project **A alone** triggers the hazard; sub-project C is not required |
| The fix guards both lookups alongside the due check, so a profile-less user is skipped and logged | [EXISTING] | `digest/runner.py:29-38` (post-fix, commit `a5a4117`) | **OQ-C-001 is resolved.** C may create an `InterestProfile` for an anonymous user without risking digest delivery |
...
- **Cross-package regression test (mandatory, per OQ-C-001)** — `tests/digest/test_runner.py` addition: `test_select_due_users_does_not_raise_for_user_with_interests_but_no_delivery_preference`. This asserts the exact hazard C introduces — a user with an `InterestProfile` and a `ProfileTopic` but no `DeliveryPreference` must be skipped, not propagate `NoResultFound` out of `select_due_users` (`digest/runner.py:32`, `digest/profiles.py:69`). It MUST be written before the first C interest-write path ships, and it MUST fail against today's code so the fix is proven.
...
**OQ-C-001 — RESOLVED 2026-08-03 (Option B), commit `a5a4117`**
Decision: How does an anonymous user acquire an `InterestProfile` without breaking the weekly digest run?

**Provenance note — the original citations were correct; a later correction to them was not.**

The first draft described `select_due_users` as already catching `NoResultFound` from `list_interests`, with the comment "Frontend-only anonymous users intentionally have no digest profile". That is **exactly true of `codex/frontend-shell`**, the branch the design task designated as its reference: the comment sits at `digest/runner.py:34` there, `get_delivery_preference` at `:38`, and the `try` at `:39` — matching the original citations line for line. A second pass re-verified against `main` instead of the reference branch, found no such code, and wrongly recorded the citations as fabricated. That retraction is itself withdrawn here.

What is true of both branches, and is the point that matters:

- On `codex/frontend-shell`, Codex independently hit this defect during its Task 10 and guarded **only** `list_interests`. `get_delivery_preference` at `:38` remained outside the `try`, so a user with an `InterestProfile` but no `DeliveryPreference` — precisely what C would create — still raised and still aborted the run. The original hazard analysis was correct.
- On `main`, **neither** call was guarded, so the defect was strictly worse and already live: sub-project A's `POST /users/anonymous` creates users with `status='active'` and no `InterestProfile`, so `list_interests` raised on the first of them. Eight such users are committed in the dev database, and six digest runner tests were failing on `main` as a direct result.

In both cases `run_all_due_digests` calls `select_due_users` under `try`/`finally` with no `except` (`digest/runner.py:85-87`), so the exception aborted delivery for every user. Commit `a5a4117` guards **both** lookups, which is what neither branch had.

Resolution: Option B, implemented in commit `a5a4117`. Both lookups now sit inside the `try` alongside the due check, so a user missing either row is skipped and logged rather than fatal. Two regression tests were added — `test_select_due_users_skips_user_with_no_interest_profile` and `test_select_due_users_skips_user_with_interests_but_no_delivery_preference` — both confirmed failing with `NoResultFound` before the change and passing after. Backend suite went from 23 failed/149 passed to 17 failed/157 passed, with no new failures.

Options A and C, recorded for the archive: Option A would have set `User.status` to a non-`active` value such as `frontend_only`, which `select_due_users` filters on at `digest/runner.py:27` — rejected because overloading `status` to mean "do not deliver" is a semantic change to a column other code reads, and it would have left the underlying fragility in place. Option C would have given C its own `dashboard_interests` table — still available if the owner later prefers strict package isolation, but no longer required for safety.

Consequence of the resolution: **C is unblocked.** It may create an `InterestProfile` for an anonymous user on first interest add without risking digest delivery. Whether C also creates a `DeliveryPreference` is now a free design choice rather than a safety requirement.
Required decision-maker: Repository owner — decision made and implemented.
