...
**Deliberately excluded, with reasons.** The per-`user_id` rate limit on the saved-search endpoint (spec §12) is **not** implemented here. `enterprise_api/rate_limit.py` is per-organization and table-backed, and generalizing it needs its own design — the roadmap already identifies a shared limiter as cross-cutting work. Task 7 instead caps the client at 10 rows, which bounds the load from Athena's own UI but not from a direct API caller. **This is a real gap, and it should be raised with the user rather than silently shipped** — it is listed here so a reviewer sees it as a decision, not an oversight. The topic-detail sparkline column and mobile column-shedding (spec §8) are also omitted; they depend on the table redesign the spec assumes and are not worth blocking the primary surfaces.
